...
# ──────────────────────────────────────
# Vision MIME autodetect
# ──────────────────────────────────────

def test_detect_png_magic_bytes():
    from ingest.services.vision import _detect_image_mime
    png = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100
...
def test_detect_gif_magic_bytes():
    from ingest.services.vision import _detect_image_mime
    assert _detect_image_mime(b"GIF89a" + b"\x00" * 100) == "image/gif"
    assert _detect_image_mime(b"GIF87a" + b"\x00" * 100) == "image/gif"
...
def test_detect_unknown_falls_back_to_png():
    from ingest.services.vision import _detect_image_mime
    assert _detect_image_mime(b"garbage12345678") == "image/png"


def test_detect_tiny_blob_returns_default():
    from ingest.services.vision import _detect_image_mime
    assert _detect_image_mime(b"") == "image/png"
    assert _detect_image_mime(b"short") == "image/png"


@pytest.mark.asyncio
async def test_parse_page_image_passes_correct_mime_for_jpeg():
    """Regression (2026-04-17): JPEG DOCX image must be sent as image/jpeg,
    not hardcoded image/png which caused Anthropic 400 errors.
    """
    ...
    async def capture_ainvoke(messages):
        # messages[0].content is a list; first item is the image block
    ...
    with patch("ingest.services.vision._get_vision_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(side_effect=capture_ainvoke)
        jpeg_bytes = b"\xff\xd8\xff\xe0" + b"\x00" * 100
        await parse_page_image(jpeg_bytes)
